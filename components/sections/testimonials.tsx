...
function TestimonialCard({ testimonial, index }: { testimonial: typeof testimonials[0], index: number }) {
  return (
    <motion.div
      ...
      className="group bg-[#0F0E0D] border border-white/5 p-10 rounded-[2rem] hover:border-accent/20 transition-all duration-500 relative flex flex-col h-full shadow-sm"
    >
      ...
      <p className="text-xl leading-relaxed mb-12 flex-grow text-[#EDE8DF]/90 font-light tracking-tight">
      ...
      <div className="flex items-center gap-5 pt-8 border-t border-white/5">
        ...
        <div className="flex flex-col">
          <span className="font-bold text-xl text-[#EDE8DF] leading-tight mb-1">
            {testimonial.name}
          </span>
      ...
      <div className="container mx-auto px-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-24 mb-24 items-end">
          <div className="max-w-xl">
            <span className="label-mono mb-6 block text-accent/60 uppercase tracking-[0.3em]">Direct Feedback</span>
            <h2 className="text-5xl md:text-6xl font-black mb-0 leading-tight tracking-tighter">
          ...
          <div className="flex items-center gap-6">
            <div className="flex -space-x-4">
              {Object.values(testimonials).slice(0, 4).map((t, i) => (
                <div key={i} className="w-10 h-10 rounded-full border-2 border-[#0A0908] overflow-hidden shadow-lg hover:z-10 transition-all duration-300">
                  <img src={t.avatar} alt="Client" className="w-full h-full object-cover" />
                </div>
              ))}
              <div className="w-10 h-10 rounded-full border-2 border-[#0A0908] bg-accent/20 flex items-center justify-center text-[10px] font-black text-accent-foreground shadow-lg backdrop-blur-sm relative z-0">
                +
              </div>
            </div>
            <p className="text-sm opacity-40 font-mono tracking-wide uppercase">
              Joined by 28+ local partners in Agra
            </p>
          </div>
          
          <div className="flex items-center gap-4 bg-[#11100F] border border-accent/20 px-8 py-4 rounded-2xl shadow-xl shadow-accent/5 backdrop-blur-3xl group hover:border-accent/40 transition-colors duration-500">
            <div className="flex gap-1 text-accent">
              {[...Array(5)].map((_, i) => (
                <Star key={i} size={14} fill="currentColor" />
              ))}
            </div>
            <span className="text-sm font-bold text-[#EDE8DF]">4.9 on Google Business</span>
          </div>
        </div>
      </div>
    </section>
